...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		// 我们的BeanFactory实际是DefaultListableBeanFactory实例，而DefaultListableBeanFactory实现了BeanDefinitionRegistry，所以是True
		if (beanFactory instanceof BeanDefinitionRegistry) {
			BeanDefinitionRegistry registry = (BeanDefinitionRegistry) beanFactory;
			// 用来存放BeanFactoryPostProcessor
			List<BeanFactoryPostProcessor> regularPostProcessors = new ArrayList<>();
			// 用来存放BeanDefinitionRegistryPostProcessor，BeanDefinitionRegistryPostProcessor继承了用来存放BeanFactoryPostProcessor
			// 这两个集合就是做了个区分，后面逻辑需要
			...
			// 就是从我们的beanFactory的beanDefinitionNames中找到类型是BeanDefinitionRegistryPostProcessor的后置处理器
			...
			for (String ppName : postProcessorNames) {
				if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) {
					...
					// beanFactory.getBean也是个很重要的方法，这个之后说，可以理解为为我们返回实例
					currentRegistryProcessors.add(beanFactory.getBean(ppName, BeanDefinitionRegistryPostProcessor.class));
					processedBeans.add(ppName);
				}
			}
			// 不是重点，可以跳过，就是给currentRegistryProcessors排序 当前我们这里只有一个ConfigurationClassPostProcessor实例
			...
			// 再次从我们的beanFactory的beanDefinitionNames中找到类型是BeanDefinitionRegistryPostProcessor的后置处理器
			...
			// 这里算第三步，执行剩余的BeanDefinitionRegistryPostProcessor，也就是没有实现Ordered接口的BeanDefinitionRegistryPostProcessor
			...
			// 前面几个步骤只执行了BeanDefinitionRegistryPostProcessor的回调，但还没有执行BeanFactoryPostProcessors的回调，此处执行
		...
		// 前面取的是实现BeanDefinitionRegistryPostProcessor的后置处理器，这里获取实现BeanFactoryPostProcessor的后置处理器
		String[] postProcessorNames =
